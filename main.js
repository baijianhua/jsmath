...
//js还是太蹩脚了，没有类型系统，要虚拟一个，没有变量是否定义的检查，没有private
...
class Coord{
    ...
    /**绘制坐标系*/
     paint(){
         ...
         //ctx.beginPath()
         //计算x轴纵坐标
         this._baseXinPx = this.marginInPixel + (totalXUnits - Math.abs(this.minX))* this.pxPerUnit; //最大是maxY个刻度，每个刻度是多少
        ...
        for(let x = this.minX; x <= totalXUnits; x++){
            this.drawLine(ctx,x,null);
         }
         for(let y = this.minY; y<= totalYUnits; y++){
            this.drawLine(ctx,null,y);
         }
    ...
    drawLine(ctx,x,y){
        ...
        if(y == null)
        {
            ...
            //纵贯画布
            startX = endX = this.marginInPixel + this._baseYinPx + y * this.pxPerUnit + 0.5;
